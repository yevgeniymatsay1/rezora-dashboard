-- Fix the expired listing specialist template by adding the missing states with user's exact configurations
UPDATE agent_templates 
SET default_settings = jsonb_set(
  default_settings,
  '{states}',
  (
    SELECT jsonb_agg(state_obj ORDER BY state_order)
    FROM (
      -- Keep existing warm_intro state first
      SELECT jsonb_array_elements(default_settings->'states') as state_obj, 1 as state_order
      FROM agent_templates 
      WHERE template_type = 'expired_listing_specialist'
      
      UNION ALL
      
      -- Add schedule_meeting state with user's exact configuration
      SELECT '{
        ...
        "edges": [],
        "tools": []
      }'::jsonb as state_obj, 2 as state_order
      
      UNION ALL
      
      -- Add callback state with user's exact configuration
      SELECT '{
        "name": "callback",
        ...
        "edges": [],
        "tools": []
      }'::jsonb as state_obj, 3 as state_order
    ) states_union
  )
)
WHERE template_type = 'expired_listing_specialist';